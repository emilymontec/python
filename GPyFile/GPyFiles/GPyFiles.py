...
def crear_archivo(nombre):
    ...
    print(f"Archivo '{os.path.basename(nombre)}' creado.")
...
def crear_carpeta(nombre):
    ...
    print(f"Carpeta '{os.path.basename(nombre)}' creada.")
...
def eliminar(ruta):
    ...
    if os.path.abspath(ruta).startswith(PAPELERA_DIR):
        confirmacion = input(f"¿Seguro que quieres eliminar '{os.path.basename(ruta)}' permanentemente? (s/n): ").lower()
        if confirmacion == 's':
            if os.path.isdir(ruta):
                shutil.rmtree(ruta)
            else:
                os.remove(ruta)
            print(f"'{os.path.basename(ruta)}' eliminado permanentemente.")
        else:
            print("Eliminación cancelada.")
    else:
        # Mueve el archivo o carpeta a la papelera
        shutil.move(ruta, os.path.join(PAPELERA_DIR, os.path.basename(ruta)))
        print(f"'{os.path.basename(ruta)}' movido a la papelera.")
...
def renombrar(ruta, nuevo_nombre):
    ...
    print(f"Renombrado '{os.path.basename(ruta)}' a '{os.path.basename(nuevo_nombre)}'.")
...
def mover(origen, destino):
    ...
    print(f"'{os.path.basename(origen)}' movido a '{os.path.basename(destino)}'.")

def put_contenido(nombre_archivo, contenido):
    nombre_archivo = resolve_path(nombre_archivo)
    if not os.path.exists(nombre_archivo):
        print(f"El archivo '{os.path.basename(nombre_archivo)}' no existe. Créalo primero.")
        return
    ...
    if extension == ".docx":
        ...
        try:
            ...
            print(f"Error: El archivo '{os.path.basename(nombre_archivo)}' no contiene un JSON válido. No se pudo agregar contenido.")
            return
        ...
        return
    print(f"Contenido agregado a '{os.path.basename(nombre_archivo)}'.")

def view_contenido(nombre_archivo):
    nombre_archivo = resolve_path(nombre_archivo)
    if not os.path.exists(nombre_archivo):
        print(f"El archivo '{os.path.basename(nombre_archivo)}' no existe.")
        return

    extension = os.path.splitext(nombre_archivo)[1].lower()
    print(f"\n--- Contenido de {os.path.basename(nombre_archivo)} ---\n")
    ...
    try:
        ...
        print(f"No se pudo leer el archivo '{os.path.basename(nombre_archivo)}': {e}")
